...
async function createTenant2User() {
  console.log('🔧 为租户二创建普通用户账户...');
  ...
  try {
    ...
    if (existingUsers.length > 0) {
      console.log('⚠️ 租户二的test_user账户已存在');
      await connection.end();
      return;
    }
    ...
    const [result] = await connection.execute(
      ...
      [
        ...
        'test_user',
        passwordHash,
        '13800138003',  // 为普通用户使用不同的手机号
        'test_user_tenant2@example.com',
        '租户二测试用户',
        'tenant_user',
        1
      ]
    ...
    console.log(`✅ 成功创建租户二的普通用户账户，ID: ${result.insertId}`);
    ...
    console.log('✅ 为普通用户账户创建了资金账户');

    await connection.end();
    console.log('✅ 租户二普通用户账户创建完成');
  } catch (error) {
    console.error('❌ 创建租户二普通用户账户失败:', error.message);
  }
}
